...
async function downloadTikTokVideo(url, outputPath) {
    return new Promise((resolve, reject) => {
        const command = `yt-dlp -o "${outputPath}" --user-agent "Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:109.0) Gecko/20100101 Firefox/109.0" --referer "https://www.tiktok.com" ${url}`;
...
client.on('message', async (message) => {
    ...
    if (url.startsWith('https://www.tiktok.com/') || url.startsWith('https://vm.tiktok.com/')) {
        const outputPath = path.join(__dirname, 'videos', `tiktok_video_${Date.now()}.mp4`);
        ...
        try {
            ...
            await client.sendMessage(message.from, "Starting download for your TikTok video... ⏳");
            ...
            if (fs.existsSync(outputPath)) {
                ...
                await client.sendMessage(message.from, "Here’s your TikTok video! 🎉");
		await client.sendMessage(message.from, "Subscribe to my YouTube Channel https://www.youtube.com/@mtaanimation0");
            } else {
